...
class ConfigVFDDisplay(Screen, ConfigListScreen):
	def __init__(self, session):
		...
		print '[VFD-Icons plugin] ADB variant:', self.adb_mod
...
#		self.DTSAvailable = False
		...
		try:
			...
			pass
		print '[VFD-Icons] ADB variant:', self.adb_model
...
def Plugins(**kwargs):
	l = [PluginDescriptor(
		...
		PluginDescriptor(
		name = _("VFD-Icons"),
		description = _("VFD-Icons for ADB ITI-5800S(X)"),
		where = PluginDescriptor.WHERE_SESSIONSTART,
		fnc = main)]
	if config.plugins.vfdicon.extMenu.value:
		l.append(PluginDescriptor(
			name = _("Front panel display"),
			description = _("Front panel display configuration for ADB ITI-5800S(X)"),
